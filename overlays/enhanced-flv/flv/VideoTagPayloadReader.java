...
/* package */ final class VideoTagPayloadReader extends TagPayloadReader {
  ...
  // Packet types.
  private static final int VIDEO_PACKET_TYPE_SEQUENCE_HEADER = 0;
  private static final int VIDEO_PACKET_TYPE_NALU = 1;
  ...
  protected boolean parsePayload(ParsableByteArray data, long timeUs) throws ParserException {
    ...
    // Parse sequence header in case this was not done before.
    if (packetType == VIDEO_PACKET_TYPE_SEQUENCE_HEADER && !hasOutputFormat) {
      output.format(parseVideoSequenceHeader(data));
      hasOutputFormat = true;
      return false;
    } else if (packetType == VIDEO_PACKET_TYPE_NALU && hasOutputFormat) {
      boolean isKeyframe = frameType == VIDEO_FRAME_KEYFRAME;
      if (!hasOutputKeyframe && !isKeyframe) {
        return false;
      }
      int bytesWritten = writeNalUnitsWithStartCodes(data);
  ...
  private Format parseVideoSequenceHeader(ParsableByteArray data) throws ParserException {
    ParsableByteArray videoSequence = new ParsableByteArray(new byte[data.bytesLeft()]);
    data.readBytes(videoSequence.getData(), 0, data.bytesLeft());
    Format.Builder formatBuilder =
        new Format.Builder().setContainerMimeType(MimeTypes.VIDEO_FLV);
    ...
    return formatBuilder
        ...
        .setMaxNumReorderSamples(hevcConfig.maxNumReorderPics)
        .setColorInfo(
            new ColorInfo.Builder()
                .setColorSpace(hevcConfig.colorSpace)
                .setColorRange(hevcConfig.colorRange)
                .setColorTransfer(hevcConfig.colorTransfer)
                .setLumaBitdepth(hevcConfig.bitdepthLuma)
                .setChromaBitdepth(hevcConfig.bitdepthChroma)
                .build())
        .build();
  }

  private int writeNalUnitsWithStartCodes(ParsableByteArray data) {
